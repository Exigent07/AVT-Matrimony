  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        month_caption: cn(
          "relative z-10 flex w-full items-center justify-center px-10",
      ...
      components={{
        ...
        MonthCaption: ({ ...props }) => <CalendarMonthCaption {...props} />,
...
function CalendarMonthCaption({
  ...
  const startMonth =
    dayPickerProps.startMonth ?? new Date(currentMonth.getFullYear() - 100, 0, 1)
  const endMonth =
    dayPickerProps.endMonth ?? new Date(currentMonth.getFullYear() + 50, 11, 1)
  const localeCode = dayPickerProps.locale?.code
  const monthLabelFormatter = new Intl.DateTimeFormat(localeCode, {
    month: "long",
  })
  const monthOptions = Array.from({ length: 12 }, (_, monthIndex) => {
    ...
    return {
      ...
      label: monthLabelFormatter.format(optionDate),
      disabled,
    }
  })
  ...
  return (
    <div className={cn("calendar-caption", className)} {...props}>
      <Select
        value={currentMonth.getMonth().toString()}
        onValueChange={(nextMonth) => {
          goToMonth(new Date(currentMonth.getFullYear(), Number(nextMonth), 1))
        }}
      >
        <SelectTrigger
          size="sm"
          aria-label="Select month"
          className="calendar-dropdown-trigger calendar-dropdown-trigger--month"
        >
          <SelectValue />
        </SelectTrigger>
        <SelectContent
      ...
      <Select
        ...
        onValueChange={(nextYear) => {
          goToMonth(new Date(Number(nextYear), currentMonth.getMonth(), 1))
        }}
      >
        <SelectTrigger
          size="sm"
          aria-label="Select year"
          className="calendar-dropdown-trigger calendar-dropdown-trigger--year"
        >
          <SelectValue />
        </SelectTrigger>
        <SelectContent
